...
class LaFTerUFT(nn.Module):
    ...
    def __init__(self, model, classes, templates, ds_templates=None, device='cuda', log=None, dataset_name=None, txt_cls=None, cfg=None, processor=None):
        ...
        self.classifier = nn.Sequential(nn.Linear(int(self.backbone_out_size), len(classes), bias=False)).to(device)
        self.deep_classifier = nn.Sequential(nn.Linear(int(self.backbone_out_size), self.deep_hidden_size),  # Input layer to hidden layer
                                            nn.ReLU(),                           # ReLU activation function
                                            nn.Linear(self.deep_hidden_size, len(classes)))
    ...
    def train_txt_clas(self, criteria):
        ...
        feas = (self.deep_classifier(txt_feas.to(torch.float32) + noise.cuda()))
    ...
    def test_txt_clas(self, image):
        ...
        txt_label = self.labels_for_text_cls
        feas = (self.deep_classifier(txt_feas.to(torch.float32)))
        return feas
    ...
    def eval_clip(self, x):
        with torch.no_grad():
            ...
            img_features_adapter = self.deep_classifier(img_features_2)
    ...
    def txt_cls_init(self):
        import copy
        self.adapter_pl = copy.deepcopy(self.deep_classifier)
